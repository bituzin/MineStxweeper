...
(define-public (calculate-rewards (game-id uint))
  (let
    (
      (game (unwrap! (contract-call? .game-core get-game-info game-id) ERR_NOT_AUTHORIZED))
      ...
      ;; Streak multiplier
      (streak-mult (unwrap! (contract-call? .player-profile get-streak-multiplier player) u1))
