...
(define-public (submit-score 
  ...
  (let
    (
      ...
      (current-size (default-to u0 (get size (default-to {size: u0} (map-get? leaderboard-sizes {difficulty: difficulty})))))
    ...
    (match current-wr
      existing-wr (if (< time (get time existing-wr))
        (begin
          ...
          (try! (contract-call? .achievement-nft award-achievement player u12)) ;; WR achievement
          (ok true)
        )
        (ok false)
      )
