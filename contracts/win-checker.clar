...
(define-public (check-win-condition (game-id uint))
  (let
    (
      (game (unwrap! (contract-call? .game-core get-game-info game-id) ERR_GAME_NOT_FOUND))
      (stats (unwrap! (contract-call? .game-core get-game-stats game-id) ERR_GAME_NOT_FOUND))
    ...
    (if (is-eq cells-revealed safe-cells)
      (let
        ...
        ;; Mark game as won
        (try! (contract-call? .game-core mark-game-won game-id final-score))
        
        ;; Trigger rewards and updates
...
(define-read-only (is-game-won (game-id uint))
  (match (contract-call? .game-core get-game-info game-id)
