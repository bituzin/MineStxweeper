...
(define-public (check-win-condition (game-id uint))
  (let
    (
      ...
      (game (unwrap! (contract-call? .game-core-04 get-game-info game-id) ERR_GAME_NOT_FOUND))
      ;; Get game statistics
      (stats (unwrap! (contract-call? .game-core-04 get-game-stats game-id) ERR_GAME_NOT_FOUND))
    ...
    (if (is-eq cells-revealed safe-cells)
      (let
        ...
        (try! (contract-call? .game-core-04 mark-game-won game-id final-score))
        ...
        (try! (contract-call? .economy-04 calculate-rewards game-id))
...
(define-read-only (is-game-won (game-id uint))
  ...
  (match (contract-call? .game-core-04 get-game-info game-id)
