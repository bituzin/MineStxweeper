...
(define-public (generate-daily-challenge)
  (let
    (
      ...
      (blocks-since (- block-height last-date))
    )
    ...
    (let
      (
        ...
        (random-val (mod block-height u10))
      ...
      (var-set last-challenge-date block-height)
...
(define-public (complete-daily-challenge (challenge-id uint) (game-id uint))
  (let
    ...
    (try! (update-daily-streak player))
      ...
      (let
        ...
        (map-set player-daily-streaks
          {player: player}
          {
            ...
            last-completion-date: block-height,
        ...
        (if (is-eq new-streak u30)
          (try! (contract-call? .achievement-nft award-achievement player u15)) ;; Daily Grinder
          true
        )
      )
      ;; Streak broken - reset
...
(define-public (claim-daily-reward (challenge-id uint))
  (let
    ...
    (let
      ...
      ;; Add rewards
      (try! (contract-call? .economy add-pending-rewards tx-sender total-reward u0))
